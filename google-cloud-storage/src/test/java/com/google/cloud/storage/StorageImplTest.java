...
public class StorageImplTest {
  ...
  public void testUploadFile() throws Exception {
    ...
    BlobInfo info = initializeUpload(dataToSend);
  ...
  public void testUploadWithOptions() throws Exception {
    ...
    BlobInfo info = initializeUpload(dataToSend, DEFAULT_BUFFER_SIZE, KMS_KEY_NAME_OPTIONS);
  ...
  public void testUploadWithBufferSize() throws Exception {
    ...
    BlobInfo info = initializeUpload(dataToSend, bufferSize);
    storage.upload(info, stream, bufferSize);
  }
  ...
  public void testUploadWithBufferSizeAndOptions() throws Exception {
    ...
    BlobInfo info = initializeUpload(dataToSend, bufferSize, KMS_KEY_NAME_OPTIONS);
    storage.upload(info, stream, bufferSize, BlobWriteOption.kmsKeyName(KMS_KEY_NAME));
  }
  ...
  public void testUploadWithSmallBufferSize() throws Exception {
    ...
    BlobInfo info = initializeUpload(dataToSend, MIN_BUFFER_SIZE);
    storage.upload(info, stream, smallBufferSize);
  }
  ...
  public void testUploadWithException() throws Exception {
    ...
    System.arraycopy(bytes, 0, buffer, 0, bytes.length);
    BlobInfo info = BLOB_INFO1.toBuilder().setMd5(null).setCrc32c(null).build();
    EasyMock.expect(storageRpcMock.open(info.toPb(), EMPTY_RPC_OPTIONS)).andReturn(uploadId);
  ...
  public void testUploadMultipleParts() throws Exception {
    ...
    storageRpcMock.write(
        EasyMock.eq(uploadId),
        EasyMock.aryEq(buffer1),
        EasyMock.eq(0),
        EasyMock.eq(0L),
        EasyMock.eq(MIN_BUFFER_SIZE),
        EasyMock.eq(false));
    ...
    storageRpcMock.write(
        EasyMock.eq(uploadId),
        EasyMock.aryEq(buffer2),
        EasyMock.eq(0),
        EasyMock.eq((long) MIN_BUFFER_SIZE),
        EasyMock.eq(extraBytes),
        EasyMock.eq(true));
