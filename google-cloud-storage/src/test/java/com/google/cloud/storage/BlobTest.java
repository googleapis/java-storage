...
public class BlobTest {
    ...
    public void testContentWithDecryptionKey() throws Exception {
        ...
        verify(storage).getOptions();
        verify(storage, times(2)).readAllBytes(BLOB_INFO.getBlobId(), Storage.BlobSourceOption.decryptionKey(BASE64_KEY));
    }
    ...
    public void testReaderWithDecryptionKey() throws Exception {
        ...
        verify(storage).getOptions();
        verify(storage, times(2)).reader(BLOB_INFO.getBlobId(), Storage.BlobSourceOption.decryptionKey(BASE64_KEY));
    ...
    public void testDeleteAcl() throws Exception {
        initializeExpectedBlob();
        when(storage.getOptions()).thenReturn(mockOptions);
        when(storage.deleteAcl(BLOB_INFO.getBlobId(), User.ofAllAuthenticatedUsers()))
                .thenReturn(true);
        initializeBlob();
