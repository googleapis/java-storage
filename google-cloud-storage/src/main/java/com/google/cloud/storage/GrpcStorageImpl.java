...
final class GrpcStorageImpl extends BaseService<StorageOptions> implements Storage {
  ...
  public Bucket lockRetentionPolicy(BucketInfo bucket, BucketTargetOption... options) {
    ...
    GrpcCallContext grpcCallContext =
        opts.grpcMetadataMapper().apply(GrpcCallContext.createDefault());
    LockBucketRetentionPolicyRequest.Builder builder = LockBucketRetentionPolicyRequest.newBuilder()
        .setBucket(bucketNameCodec.encode(bucket.getName()));
    LockBucketRetentionPolicyRequest req = opts.LockBucketRetentionPolicysRequest().apply(builder).build();
    return Retrying.run(
        getOptions(),
        retryAlgorithmManager.getFor(req),
